...
module.exports = () => {
   ...
   *     description: Create new project. Restricted to admin/manager.
   *     security:
   *       - bearerAuth: []
   *     parameters:
   *       - name: projectName
   *         description: Project name. Max 255 chars.
   *         in: query
   *         required: true
   *         type: string
   *       - name: description
    ...
    async (req, res) => {
      if (req.user['role'] > 2) {
        return res
          .status(200)
          .json(
            gg.returnDat(true, 400, 'Your role cannot add projects.', null)
          );
      }
        ...
        .then(function (data) {
          ...
          if (data == null || !data) {
            ...
            json = {
              error: true,
              code: 400,
              message: 'Active project found for that name.',
              data: null,
            }; //or mobile
          }
          return json;
      ...
      const dat2 = await global.Models.projects
        .create({
        ...
        .then(function (data) {
          let infoDat = {
            ...
            location: 
                data.dataValues.locationAddress + ', ' + data.dataValues.locationCity + ', ' 
                + data.dataValues.locationState + ', ' + data.dataValues.locationCountry 
